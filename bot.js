...
function respond() {
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    this.res.writeHead(200);
    //postMessage("http://www.daddyleagues.com/p2w?name=&position=all&team="+request.text.substring(5,8));
    postMessage("http://daddyleagues.com/p2w/team/"+request.text.substring(5,8)+"/depthchart");
    this.res.end();
  } 
  
  else if(request.text && botRegexOW.test(request.text)) {
    this.res.writeHead(200);
    postMessage("www.daddyleagues.com/p2w/");
    this.res.end();
  } 
    ...
    postMessage("http://daddyleagues.com/p2w/team/"+request.text.substring(5,8)+"/schedule");
    this.res.end();
  }
    ...
    var rep = req.replace(/ /,"+");
    postMessage("http://daddyleagues.com/p2w/players?name="+rep+"&position=all&team=all");
